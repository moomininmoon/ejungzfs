...
#define PID_FILE "/var/run/zfs_memory_monitor.pid"
#define CONTROL_FILE "/etc/zfs_memory_monitor_control"
...
static zfs_command_t command_table[] = {
	...
	{ "m", 		zfs_do_memory_monitor,  HELP_MEMORY 		},
	{ NULL },
...
get_usage(zfs_help_t idx)
{
	...
	case HELP_MEMORY:
		return (gettext("\tm\t\tDisplay system memory usage and mail to you :) lol!!\n"));
	case HELP_VERSION:
...
static double get_memory_usage() {
    ...
    if (meminfo == NULL) {
	syslog(LOG_ERR, "Error opening meminfo");
	exit(1);
    }

    char line[256];
    double total_memory = 0, free_memory = 0;
...
static void *memory_monitor_thread(void *arg) {
	...
	if (control) {
	    char status[10];
	    if (fgets(status, sizeof(status), control) != NULL) {
		if (strncmp(status, "off", 3) == 0) {
		    syslog(LOG_INFO, "ZFS memory monitor turned off");
		    fclose(control);
		    sleep(60);
		    continue;
		}
	...
	syslog(LOG_INFO, "Current ZFS memory usage: %.2f%%", memory_usage);

	if (memory_usage > MEMORY_THRESHOLD) {
	    syslog(LOG_WARNING, "ZFS memory usage exceeds threshold. Sending alert...");
...
main(int argc, char **argv)
{
	...
	if (strcmp(argv[1], "m") == 0) {
	    return zfs_do_memory_monitor(argc, argv);
	}
	/*
